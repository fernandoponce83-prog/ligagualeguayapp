...
// ── GALERÍA ───────────────────────────────────────────────────
// Hoja: "galeria" — col: nombre, descripcion, url_foto, categoria, activo
function getGaleria(ss) {
  var sheet = getSheet(ss, ['galeria','Galeria','galería','Galería']);
  if (!sheet) return [];
  var rows = getRows(sheet);
  return rows
    .filter(function(r){ return boolVal(r['activo'] !== undefined ? r['activo'] : 'si'); })
    .map(function(r) {
      return {
        nombre:      strVal(r['nombre'] || r['titulo'] || r['título'] || ''),
        descripcion: strVal(r['descripcion'] || r['descripción'] || ''),
        url_foto:    strVal(r['url_foto'] || r['url'] || r['foto'] || ''),
        categoria:   strVal(r['categoria'] || r['fecha'] || ''),
      };
    })
    .filter(function(r){ return r.url_foto !== ''; });
}
